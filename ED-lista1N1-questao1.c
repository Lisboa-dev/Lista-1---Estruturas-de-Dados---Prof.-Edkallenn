...
int *potencia_fatorial_crescente(int n, int p);
int ** matriz_fatorial_crescente(int n,int range_n, int p, int range_p);
int * potencia_fatorial_decrescente(int n, int p);
int **matriz_fatorial_decrescente(int n,int range_n, int p, int range_p);
void exibir_functions();
...
int *potencia_fatorial_crescente(int n,int p ){
 //n se trata da base do fatorial e p seria um limite pro crescimento
  ...
  if(matriz_crescente!=NULL){
    printf("alocação de matriz_crescente na primeira dimensão bem sucedida");
  }else{
...
// os ranges são valores inciais enquanto, o n e p, são os valores finais. isso gera um gap de valores.
 ...
 void exibe_matriz_fatorial(int **matriz, int diferenca_ran_n, int diferenca_ran_p )
...
int main(){
  ...
  matrizI= matriz_fatorial_crescente(5,0,5,0);
  matriz= matriz_fatorial_decrescente(5,0,5,0);
